<?php
if(!defined("InMall")) {
    exit("Access Invalid!");
}
require_once MALL_ROOT.'/vendor/autoload.php';
use Yansongda\Pay\Pay;
class agent_cashControl extends BaseProfileControl {
    public function indexOp() {
        if(submitcheck()) {
            $agent_id=empty($_POST['agent_id']) ? 0 : $_POST['agent_id'];
            if(empty($agent_id)){
                exit(json_encode(array('msg'=>'机构不存在')));
            }
            $agent=DB::fetch_first("SELECT * FROM ".DB::table('agent')." WHERE agent_id='$agent_id'");
            if(empty($agent)){
                exit(json_encode(array('msg'=>'机构不存在')));
            }
            $pdc_amount = empty($_POST['pdc_amount']) ? 0 : intval($_POST['pdc_amount']);

            $plat_amount = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND profit_type=1");
            $plat_refund = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND profit_type=0");
            $pool_amount = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND is_freeze=1");
            $income = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND profit_type=1 AND is_freeze=0");
            $expend = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND profit_type=0 AND is_freeze=0");
            $available_amount = $income-$expend;

            if(empty($pdc_amount)) {
                exit(json_encode(array('msg'=>'请输入提现金额')));
            }
            if($pdc_amount < 10) {
                exit(json_encode(array('msg'=>'提现金额不能小于10')));
            }
            if($pdc_amount > 50000) {
                exit(json_encode(array('msg'=>'提现金额不能大于50000')));
            }
            if($pdc_amount > $available_amount) {
                exit(json_encode(array('msg'=>'您的可提现金额只有￥'.$available_amount)));
            }
            dsetcookie('mallpdcamount', $pdc_amount, 3600);
            exit(json_encode(array('done'=>'true')));
        } else {
            $agent_id=empty($_GET['agent_id']) ? 0 : $_GET['agent_id'];
            if(empty($agent_id)){
                $this->showmessage('您还未登录了','index.php','info');
            }
            $agent=DB::fetch_first("SELECT * FROM ".DB::table('agent')." WHERE agent_id='$agent_id'");
            if(empty($agent)){
                $this->showmessage('机构不存在','index.php','error');
            }
            $plat_amount = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND profit_type=1");
            $plat_refund = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND profit_type=0");
            $pool_amount = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND is_freeze=1");
            $income = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND profit_type=1 AND is_freeze=0");
            $expend = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND profit_type=0 AND is_freeze=0");
            $available_amount = $income-$expend;
            $curmodule = 'home';
            $bodyclass = 'gray-bg';
            include(template('agent_cash'));
        }
    }

    public function step2Op() {
        if(submitcheck()) {
            $pdc_amount = dgetcookie('mallpdcamount');
            $agent_id=empty($_POST['agent_id']) ? 0 : $_POST['agent_id'];
            if(empty($agent_id)){
                exit(json_encode(array('msg'=>'机构不存在')));
            }
            $agent=DB::fetch_first("SELECT * FROM ".DB::table('agent')." WHERE agent_id='$agent_id'");
            if(empty($agent)){
                exit(json_encode(array('msg'=>'机构不存在')));
            }
            $plat_amount = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND profit_type=1");
            $plat_refund = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND profit_type=0");
            $pool_amount = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND is_freeze=1");
            $income = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND profit_type=1 AND is_freeze=0");
            $expend = DB::result_first("SELECT SUM(profit_amount) FROM ".DB::table('nurse_profit')." WHERE agent_id='$agent_id' AND profit_type=0 AND is_freeze=0");
            $available_amount = $income-$expend;
            $pdc_code = empty($_POST['pdc_code']) ? '' : $_POST['pdc_code'];
            $alipay_card = empty($_POST['alipay_card']) ? '' : $_POST['alipay_card'];
            $weixin_card = empty($_POST['weixin_card']) ? '' : $_POST['weixin_card'];
            $bank_membername = empty($_POST['bank_membername']) ? '' : $_POST['bank_membername'];
            $bank_deposit = empty($_POST['bank_deposit']) ? '' : $_POST['bank_deposit'];
            $bank_card = empty($_POST['bank_card']) ? '' : $_POST['bank_card'];
            if(empty($pdc_amount)) {
                exit(json_encode(array('msg'=>'请输入提现金额')));
            }
            if($pdc_amount < 10) {
                exit(json_encode(array('msg'=>'提现金额不能小于10')));
            }
            if($pdc_amount > 50000) {
                exit(json_encode(array('msg'=>'提现金额不能大于50000')));
            }
            if($pdc_amount > $available_amount) {
                exit(json_encode(array('msg'=>'您的可提现金额只有￥'.$available_amount)));
            }
            if($pdc_code == 'alipay') {
                if(empty($alipay_card)) {
                    exit(json_encode(array('msg'=>'请输入支付宝帐号')));
                }
            } elseif($pdc_code == 'weixin') {
                if(empty($weixin_card)) {
                    exit(json_encode(array('msg'=>'请输入微信号')));
                }
            } elseif($pdc_code == 'bank') {
                if(empty($bank_membername)) {
                    exit(json_encode(array('msg'=>'请输入收款人')));
                }
                if(empty($bank_deposit)) {
                    exit(json_encode(array('msg'=>'请输入开户行')));
                }
                if(empty($bank_card)) {
                    exit(json_encode(array('msg'=>'请输入银行卡号')));
                }
            }
//            $pdc_sn = makesn(6);
            $pdc_sn = makeoutsn(5);
            $data = array(
                'pdc_sn' => $pdc_sn,
                'agent_id'=>$agent_id,
                'member_id' => $this->member_id,
                'pdc_code' => $pdc_code,
                'pdc_amount' => $pdc_amount,
                'pdc_state' => 3,
                'add_time' => time(),
            );
            if($pdc_code == 'alipay') {
                $data['alipay_card'] = $alipay_card;
            } elseif($pdc_code == 'weixin') {
                $data['weixin_card'] = $weixin_card;
            } elseif($pdc_code == 'bank') {
                $data['bank_membername'] = $bank_membername;
                $data['bank_deposit'] = $bank_deposit;
                $data['bank_card'] = $bank_card;
            }
            $pdc_id = DB::insert('pd_cash', $data, 1);
            if(!empty($pdc_id)) {
                $member = DB::fetch_first("SELECT * FROM ".DB::table('member')." WHERE member_id='$this->member_id'");

                $nurse_profit_data=array(
                    'agent_id'=>$agent_id,
                    'profit_stage'=>'cash',
                    'profit_type'=>0,
                    'profit_amount'=>$pdc_amount,
                    'profit_desc'=>'申请提现，单号'.$pdc_sn,
                    'is_freeze'=>0,
                    'add_time'=>time()
                );
                DB::insert('nurse_profit',$nurse_profit_data);
                $agent_profit_data=array(
                    'agent_id'=>$agent_id,
                    'profit_stage'=>'cash',
                    'profit_type'=>0,
                    'profit_amount'=>$pdc_amount,
                    'profit_desc'=>'申请提现,单号'.$pdc_sn,
                    'is_freeze'=>0,
                    'add_time'=>time(),

                );
                DB::insert('agent_profit', $agent_profit_data);
                dsetcookie('mallpdcamount', '', -3600);
                /**
                 * !!!仅限测试期间使用!!!
                 * @todo 临时提现功能实现，后期必须分离内网处理！！！
                 * 2017/9/29
                 */
                $config = [
                    'alipay' => [
                        'app_id' => '2017040606574715',             // 支付宝提供的 APP_ID
                        'ali_public_key' => '', // 支付宝公钥
                        'private_key' => '' //应用私钥
                    ],
                ];
                $config_biz = [
                    'out_biz_no' => $pdc_sn,                      // 订单号
                    'payee_type' => 'ALIPAY_LOGONID',        // 收款方账户类型(ALIPAY_LOGONID | ALIPAY_USERID)
                    'payee_account' => $alipay_card,   // 收款方账户
                    'amount' => $pdc_amount,                        // 转账金额
                    'payer_show_name' => '团家政',             // 付款方姓名
//                    'payee_real_name' => '宋吕翔',             // 收款方真实姓名
                    'payee_real_name' => $member['member_truename'],             // 收款方真实姓名
                    'remark' => '团家政用户提现',                      // 转账备注
                ];
                $pay = new Pay($config);
                try {
                    $transfer = $pay->driver('alipay')->gateway('transfer')->pay($config_biz);
//                    exit(json_encode($transfer));
                    if(is_array($transfer)){
                        DB::query("UPDATE ".DB::table('pd_cash')." SET pay_sucess_id='".$transfer[order_id]."', pdc_state='1', pay_msg='付款成功' WHERE pdc_sn='".$pdc_sn."'");
                        $finance_data=array(
                            'finance_type'=>'deposit',
                            'member_id'=>$member['member_id'],
                            'finance_state'=>1,
                            'finance_amount'=>$pdc_amount,
                            'finance_time'=>time()
                        );
                        DB::insert('finance',$finance_data);
                    }else{
//                        exit(json_encode(array('msg'=>'提现申请已经提交，请等待处理')));
                    }
//                    exit($transfer);
                }catch (Exception  $e ){
                    if(!empty($e->raw['alipay_fund_trans_toaccount_transfer_response'])){
                        DB::query("UPDATE ".DB::table('pd_cash')." SET pay_msg='".json_encode($e->raw['alipay_fund_trans_toaccount_transfer_response'])."', pdc_state='2'  WHERE pdc_sn='".$pdc_sn."'");
                    }else{
                        DB::query("UPDATE ".DB::table('pd_cash')." SET pay_msg='".$e->getMessage()."', pdc_state='2'  WHERE pdc_sn='".$pdc_sn."'");
                    }
                    $nurse_profit_data_refund=array(
                        'agent_id'=>$agent_id,
                        'profit_stage'=>'cash',
                        'profit_type'=>1,
                        'profit_amount'=>$pdc_amount,
                        'profit_desc'=>'申请提现失败退款,单号'.$pdc_sn,
                        'is_freeze'=>0,
                        'add_time'=>time(),

                    );
                    DB::insert('nurse_profit',$nurse_profit_data_refund);
                    DB::insert('agent_profit', $nurse_profit_data_refund);
                }
                
                exit(json_encode(array('done'=>'true')));
            } else {
                exit(json_encode(array('msg'=>'网路繁忙，请稍候重试')));
            }
        } else {
            $agent_id=empty($_GET['agent_id']) ? 0 : $_GET['agent_id'];
            if(empty($agent_id)){
                $this->showmessage('您还未登录了','index.php','info');
            }
            $agent=DB::fetch_first("SELECT * FROM ".DB::table('agent')." WHERE agent_id='$agent_id'");
            if(empty($agent)){
                $this->showmessage('机构不存在','index.php','error');
            }
            $pdc_amount = dgetcookie('mallpdcamount');
            $cash = DB::fetch_first("SELECT * FROM ".DB::table('pd_cash')." WHERE member_id='$this->member_id' ORDER BY add_time DESC");
//            $cash['pdc_code'] = empty($cash['pdc_code']) ? 'alipay' : $cash['pdc_code'];
            $cash['pdc_code'] = 'alipay';
            $cash_type = array('alipay'=>'支付宝', 'weixin'=>'微信', 'bank'=>'银行卡');
            $curmodule = 'home';
            $bodyclass = 'gray-bg';
            include(template('agent_cash_step2'));
        }
    }
}

?>