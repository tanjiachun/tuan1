...
class cashControl extends BaseProfileControl {
	public function indexOp() {
		if(submitcheck()) {
            if($this->member['member_real_state'] != '1'){
                exit(json_encode(array('msg'=>'为了您的资金安全，提现前请先实名认证！')));
            }
			...
			if($pdc_amount > $this->member['available_predeposit']) {
				exit(json_encode(array('msg'=>'您的可提现金额只有￥'.$this->member['available_predeposit'])));
			}
