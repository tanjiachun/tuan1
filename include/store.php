...
class BaseStoreControl {
	...
	private function init_store() {
		$this->store = DB::fetch_first("SELECT * FROM ".DB::table('store')." WHERE member_id='$this->member_id'");
		$this->store_id = empty($this->store['store_id']) ? 0 : $this->store['store_id'];
		if(empty($this->store_id)) {
			$this->showmessage('您还不是商家', 'index.php?act=store&op=register', 'info');
		}
	}
