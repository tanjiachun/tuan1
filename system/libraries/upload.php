...
class Upload {
	...
	function init($attach, $type = 'temp', $forcename = '') {
		if(!is_array($attach) || empty($attach) || !$this->is_upload_file($attach['tmp_name']) || trim($attach['name']) == '' || $attach['size'] == 0) {
			...
			$attach['attachment'] = $attach['attachdir'].$this->get_target_filename($this->forcename).'.'.$attach['extension'];
			$attach['target'] = MALL_ROOT.'/data/'.$this->type.'/'.$attach['attachment'];
			$this->attach = & $attach;
			$this->errorcode = 0;
			return true;
		}
	...
	function save($ignore = 0) {
		...
		if(empty($this->attach) || empty($this->attach['tmp_name']) || empty($this->attach['target'])) {
			$this->errorcode = -101;
		} elseif($this->type != 'attachment' && !$this->attach['isimage']) {
			$this->errorcode = -102;
	...
	function errormessage() {
		$errormessage = array(
			'-1' => '上传失败！上传文件不存在或不合法，请返回。',
			'-101' => '上传失败！上传文件不存在或不合法，请返回。',
			'-102' => '上传失败！非图片类型文件，请返回。',
			'-103' => '上传失败！无法写入文件或写入失败，请返回。',
			'-104' => '上传失败！无法识别的图像文件格式，请返回。',
		);
	...
	function get_target_filename($forcename = '') {
		if($forcename != '') {
			$filename = ($forcename != '' ? $forcename : '');
		} else {
	...
	function get_target_extension($ext) {
		$safeext  = array('attach', 'jpg', 'jpeg', 'gif', 'png', 'swf', 'bmp', 'txt', 'zip', 'rar', 'mp3', 'flv', 'f4v', 'mp4');
	...
	function check_dir_type($type) {
		return !in_array($type, array('goods', 'store', 'plat', 'nurse', 'agent', 'album', 'mobile')) ? 'temp' : $type;
	}

	function check_dir_exists($type, $sub1 = '', $sub2 = '') {
		$basedir = MALL_ROOT.'/data';
