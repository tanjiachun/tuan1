<?php include(template('common_header'));?>
            ...
            <div class="pay-box">
                <div class="pay-head">
                    <div class="money">
                        <dl>
                            <dt>
                                <span>单号：<?php echo $book['book_sn'];?></span>
                                <em>
                            ...
                            <dd class="wf show">
                                ...
                                <p>保证金缴纳时间：<?php echo date('Y-m-d H:i', $book['add_time']);?> </p>
                            </dd>
                        </dl>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        ...
        function checkstate(book_sn) {
            $.getJSON('index.php?act=agent_marketing&op=weixin',{'book_sn':book_sn},function (data) {
                if(data.data.trade_state=='SUCCESS' && data.money*100==parseInt(data.data.total_fee) && data.data.return_code=='SUCCESS' && data.data.result_code=='SUCCESS'){
                    ...
                    $.getJSON('index.php?act=agent_marketing&op=weixin_book',{'book_sn':book_sn,'transaction_id':transaction_id,'total_fee':total_fee},function (data) {
                        if(data.done=='true'){
                            window.location.href='index.php?act=agent_marketing';
                        }
                    })
                }
            })
