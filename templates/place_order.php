        ...
        <div class="orderlist">
            <div class="orderlist-body">
                <table class="order-tb">
                    <input type="hidden" id="nurse_id" value="<?php echo $nurse['nurse_id'] ?>">
                    <input type="hidden" id="agent_id" value="<?php echo $nurse['agent_id'] ?>">
                    <input type="hidden" id="nurse_type" value="<?php echo $nurse['nurse_type'] ?>">
                    <input type="hidden" id="nurse_phone" value="<?php echo $nurse['member_phone'] ?>">
                    <input type="hidden" id="book_details" value="<?php echo $member_favourite['collect_details'] ?>">
                    <input type="hidden" id="work_duration" value="<?php echo $member_favourite['work_duration'] ?>">
                    <input type="hidden" id="work_duration_days" value="<?php echo $member_favourite['work_duration_days'] ?>">
                    <input type="hidden" id="work_duration_hours" value="<?php echo $member_favourite['work_duration_hours'] ?>">
                    <input type="hidden" id="work_duration_mins" value="<?php echo $member_favourite['work_duration_mins'] ?>">
                    <input type="hidden" id="work_area" value="<?php echo $member_favourite['work_area'] ?>">
                    <input type="hidden" id="work_person" value="<?php echo $member_favourite['work_person'] ?>">
                    <input type="hidden" id="work_machine" value="<?php echo $member_favourite['work_machine'] ?>">
                    <input type="hidden" id="work_cars" value="<?php echo $member_favourite['work_cars'] ?>">
                    <input type="hidden" id="car_price" value="<?php echo $member_favourite['car_price'] ?>">
                    <input type="hidden" id="work_students" value="<?php echo $member_favourite['work_students'] ?>">
                    <input type="hidden" id="service_price" value="<?php echo $member_favourite['service_price'] ?>">
                    <input type="hidden" id="nurse_price" value="<?php echo $member_favourite['nurse_price'] ?>">
                    <input type="hidden" id="deposit_amount" value="<?php echo $member_favourite['total_price'] ?>">
                    <input type="hidden" id="book_amount" value="">
                    <thead>
                    <tr>
                        ...
                        <tr class="tr-th">
                            <td colspan="5">
                                <span style="color:#000;">机构：<?php echo $agent['agent_name'];?></span>
                                <span style="color: #000;">联系电话：<?php echo $nurse['member_phone'];?></span>
<!--                                <span> <a href="javascript:;"><img style="margin: 0 5px 2px 0" src="templates/images/lianxi.png" alt="">和我联系</a></span>-->
                        ...
                        <tr class="tr-bd">
                            <td>
                                <div class="td-inner clearfix w-300">
                                    ...
                                    <div class="item-info">
                                        <a style="overflow: inherit;margin-right: 20px;" href="index.php?act=nurse&nurse_id=<?php echo $nurse['nurse_id'];?>" target="_blank"><?php echo $nurse['nurse_nickname'];?></a>
                                        <span><?php echo $nurse['nurse_special_service'];?></span>
                                    </div>
                                    <div class="item_image">
                                        <?php if($nurse['promise_state']==2) { ?>
                                            <img src="templates/images/three_days.jpg" alt="">三小时无理由
                                        <?php } elseif($nurse['promise_state']==4) { ?>
                            ...
                            <td>
                                <?php echo $member_favourite['collect_details'] ?>
                            </td>
                            <td>
                                <?php if($nurse['nurse_type']==3) { ?>
                                    每小时¥<?php echo $member_favourite['nurse_price'] ?>
                                <?php } else if($nurse['nurse_type']==4) { ?>
                                    每平方¥<?php echo $member_favourite['nurse_price'] ?>
                                <?php } else if($nurse['nurse_type']==5 || $nurse['nurse_type']==6) { ?>
                                    每月¥<?php echo $member_favourite['nurse_price'] ?><br>
                                    （以26天计）
                                <?php } else if($nurse['nurse_type']==7 || $nurse['nurse_type']==8) { ?>
                                    每次¥<?php echo $member_favourite['nurse_price'] ?>
                                <?php } else if($nurse['nurse_type']==9 || $nurse['nurse_type']==10) { ?>
                                    每人¥<?php echo $member_favourite['nurse_price'] ?><br>
                                    <?php if(!empty($member_favourite['work_cars'])) { ?>
                                        (<?php echo $member_favourite['work_cars'] ?>吨车，¥ <?php echo $member_favourite['car_price'] ?>)
                                    <?php } ?>
                                <?php } else if($nurse['nurse_type']==11 || $nurse['nurse_type']==12) { ?>
                                    每个学生每月¥<?php echo $member_favourite['nurse_price'] ?><br>
                                    ...
                                    <?php } else { ?>
                                     ...
                                     <?php } ?>
                                <?php } else { ?>
                                    服务费 ¥ <?php echo $member_favourite['service_price'] ?> <br>
                                    本月工资 ¥ <?php echo $member_favourite['nurse_price'] ?>
                                <?php } ?>
                            </td>
                            <td>
                                团家政 <span style="color: #ff6905;"><?php echo $nurse['nurse_discount']*10 ?></span> 折优惠
                            </td>
                            <td>
                                <span style="color: #ff6905;font-size: 16px;font-weight: 700;"><?php echo $member_favourite['total_price'] ?>.00</span>
                            </td>
                        </tr>
                        <tr class="tr-ft">
                            <td colspan="5" style="background: #f2f7ff;">
                                ...
                                <div style="text-align: right;">
                                    合计：<span style="color: #ff6905;font-size: 16px;font-weight: 700;">¥ <?php echo $member_favourite['total_price'] ?>.00</span>
                                </div>
                            </td>
                        </tr>
                        <tr class="tf-ft">
                        ...
                        <tr class="tr-ft">
                            <td colspan="5" style="border: none;border-bottom: 1px dotted #e5e5e5;">
                                <div class="right">
                                    <input type="checkbox" id="member_coin_discount">使用 <span class="coin_count"><?php echo $member_discount_price*100 ?></span>个团豆豆抵扣<span class="member_discount_price"><?php echo $member_discount_price ?></span>元 &nbsp;&nbsp;&nbsp;&nbsp;<span style="font-weight: 600;">¥</span> <b class="discount_price">-0.00</b>
                                    <br>
                                    <?php if(intval($member['member_coin'])/100<=$discount_price) { ?>
                                        <span style="font-size: 12px;font-weight: 300;">（你有<?php echo $member['member_coin'] ?>个，使用<?php echo $member_discount_price*100 ?>个，可抵总价<?php echo $member_discount_price ?> 元）</span>
                                    <?php } else { ?>
                                        <span style="font-size: 12px;font-weight: 300;">（你有<?php echo $member['member_coin'] ?>个，使用<?php echo $member_discount_price*100 ?>个，可抵总价2%）</span>
                                    <?php } ?>
                                </div>
                            </td>
                        </tr>
                        ...
                        <tr class="tr-ft">
                            <td colspan="5" style="border:none;padding-right: 0;padding-bottom: 0;">
                                <div class="right" style="border:1px solid #ff6905;padding:20px 60px 20px 60px;">
                                    <span style="color: #000;">实&nbsp;&nbsp;付&nbsp;&nbsp;款：</span> <span style="font-size: 18px;font-weight: 700;">¥</span> <b style="font-size: 24px;font-weight: 700;color: #ff6905;" id="true_price"><?php echo $member_favourite['total_price'] ?></b>
                                    <br>
...
<script>
    var member_discount_price='<?php echo $member_discount_price*100 ?>';
    console.log(member_discount_price);
    var total_price='<?php echo $member_favourite['total_price'] ?>';
    var member_coin='<?php echo $member['member_coin'] ?>';
    console.log(member_coin);
</script>
<script>
    $("#member_coin_discount").click(function () {
        if($(this).is(":checked")){
            console.log(1);
            var data=$(this).siblings(".coin_count").html();
            var html='<input type="number" class="choose_coin_count" min=0 value=""> ';
            $(".coin_count").html(html);
            $(".choose_coin_count").val(member_discount_price);
            $(".choose_coin_count").attr('max',member_discount_price);
            $(".discount_price").html('-'+(parseFloat(member_discount_price/100)).toFixed(2));
            $("#true_price").html((parseFloat(total_price)-parseFloat(member_discount_price/100)).toFixed(2));
            $(".choose_coin_count").on('input propertypechange',function () {
                if($(this).val()<0){
                    $(this).val(0);
                }
                if($(this).val()>member_discount_price){
                    $(this).val(member_discount_price);
                }
                if($(this).val()>parseInt(member_coin)){
                    $(this).val(parseInt(member_coin));
                }
                $(".member_discount_price").html($(this).val()/100);
                $(".discount_price").html('-'+(parseFloat($(this).val()/100)).toFixed(2));
                var true_price=(parseFloat(total_price)-parseFloat($(this).val()/100)).toFixed(2);
...
<script>
    var collect_id='<?php echo $collect_id ?>';
...
<script>
    ...
    function addsubmit() {
        ...
        if(address_content==''){
            showerror('add_address_content','请填写详细地址');
            return;
        }else{
            showsuccess('add_address_content');
        }
        if(address_member_name==''){
            showerror('add_address_member_name','请填写联系人姓名');
            return;
        }else{
            showsuccess('add_address_member_name');
        }
        if(address_phone==''){
            showerror('add_address_phone','请填写联系号码');
            return;
        }else{
            showsuccess('add_address_phone');
        }
        var reg2 = /^[1][0-9]{10}$/;
        if(!reg2.test(address_phone)){
            showerror('add_address_phone', '手机号格式不正确');
            return;
        }else{
            showsuccess('add_address_phone');
        }
...
<script>
 $(".order_submit_btn").click(function () {
     ...
     var book_phone=$("#nurse_phone").val();
     var book_details=$("#book_details").val();
     var work_duration=$("#work_duration").val();
     var work_duration_days=$("#work_duration_days").val();
     var work_duration_hours=$("#work_duration_hours").val();
     var work_duration_mins=$("#work_duration_mins").val();
     var work_area=$("#work_area").val();
     var work_person=$("#work_person").val();
     var work_machine=$("#work_machine").val();
     var work_cars=$("#work_cars").val();
     var car_price=$("#car_price").val();
     var work_students=$("#work_students").val();
     ...
     var member_coin_amount=$(".choose_coin_count").val();
     var book_amount=parseFloat($("#true_price").html());
     ...
     console.log(submitData);
//     return;
     if(book_address=='' || book_address==' ' || book_address=='   '){
         showalert('请添加工作地址');
         return;
     }
     $.ajax({
         type : 'POST',
         url : 'index.php?act=place_order&op=order',
         data : submitData,
         contentType: 'application/x-www-form-urlencoded; charset=utf-8',
         dataType : 'json',
         success : function(data) {
